    ...
    if retval != 0:
        ...
        if len(files) != num_to_find:
            logger.error("%s matched %d papiex output files instead of %d",
                         global_datadir + settings.input_pattern,
                         len(files),
                         num_to_find)
            retval = False
    if retval:
        ...
        if len(files) != 1:
            logger.error("%s matched %d job_metadata files instead of 1",
                         global_datadir + "job_metadata",
                         len(files))
            retval = False
...
def epmt_annotate(argslist, replace=False):
    ...
    def get_annotations_from_kwargs(args):
        '''
        this function returns the annotation dictionary from kwargs with some error checking
        '''
    ...
    if '=' in argslist[0]:
        # if = in first arg, then we have no filename and are annotating a job in a batch environment.
        # the job must have at least started- it might be running or stopped
        ...
        # no '=' in the first arg that means the first arg refers to a staged job file, a directory or a job in db.
...
def _papiex_opt_byhost(o):
    ...
    if not hasattr(o, 'papiex_options_byhost'):
        return ""
...
def _papiex_opt_bycpu(o):
    if not hasattr(o, 'papiex_options_bycpu'):
        return ""

    if not isinstance(o.papiex_options_bycpu, dict):
...
def epmt_source(slurm_prolog=False, papiex_debug=False, monitor_debug=False, run_cmd=False):
    ...
    def detect_csh():
        for v in [ environ.get("_"), environ.get("SHELL") ]:
            if v and v.endswith("csh"):
                logger.warning("Detected CSH - we recommend upgrading to BASH, but we'll do our best.")
                return True
        return False
    ...
    if monitor_debug:
        cmd = add_var(cmd,
                      "MONITOR_DEBUG" + equals + "TRUE")
    if papiex_debug:
        cmd = add_var(cmd,
                      "PAPIEX_DEBUG" + equals + "TRUE")
    cmd = add_var(cmd,
                  "PAPIEX_OUTPUT" + equals + global_datadir)
    cmd = add_var(cmd,
    ...
    if run_cmd:
        ...
        cmd = add_var(cmd,
                      "PAPIEX_LD_PRELOAD" + equals + papiex_pl_libs)
...
def epmt_run(cmdline, wrapit=False, dry_run=False, debug=False):
    ...
    if wrapit:
        if dry_run:
            ...
            if not epmt_stop_job(keep_going=False):
    ...
    for v in r.values():
        for (f, res) in v.items():
            ...
            if not status:
                ...
                # we may have a True status, but if submit_details is empty that means the job was already in the
                # database, and we couldn't submit it. Our behavior depends on whether keep_going is enabled or not.
...
def extract_tar(tarfile, outdir='', check_metadata=False):
    ...
    Returns
    -------
        Output directory where tar was extracted on success, False on error
    '''
...
def submit_to_db(inputf, pattern, dry_run=True):
    ...
    from epmt.orm import setup_db
    if not setup_db(settings, False):
...
def epmt_entrypoint(args):
    ...
    if args.command == 'convert':
        ...
        return not convert_csv_in_tar( args.src_tgz,
                                       args.dest_tgz )
    ...
    if args.command == 'daemon':
        ...
        if args.start or args.foreground:
            if not any( [ args.ingest , args.post_process , args.retire ] ):
    ...
    if args.command == 'drop':
        ...
        logger.warning('request to drop all data in the database')
        from epmt.orm import orm_drop_db
        orm_drop_db()
        return 0

    if args.command == 'dbsize':
        return not epmt_dbsize( args.epmt_cmd_args)

    if args.command == 'start':
        return not epmt_start_job( keep_going=not args.error,
                                   other=args.epmt_cmd_args)

    if args.command == 'stop':
        return not epmt_stop_job( keep_going=not args.error,
                                  other=args.epmt_cmd_args)

    if args.command == "stage":
        return not epmt_stage( args.epmt_cmd_args,
                               keep_going=not args.error,
                               collate=not args.no_collate,
                               compress_and_tar=not args.no_compress_and_tar)
    if args.command == 'run':
    ...
    if args.command == 'schema':
        ...
        return not orm_dump_schema()

    if args.command == 'migrate':
        from epmt.orm import setup_db
        return not setup_db(settings)
    ...
    if args.command == 'dump':
        return not epmt_dump_metadata(args.epmt_cmd_args,
                                      key=args.key)

    if args.command == 'submit':
        return not epmt_submit( args.epmt_cmd_args,
                                dry_run=args.dry_run,
                                drop=args.drop,
                                keep_going=not args.error,
                                ncpus=args.num_cpus,
                                remove_on_success=args.remove,
                                move_on_failure=args.move_away)

    if args.command == 'check':
        return not epmt_check()

    if args.command == 'delete':
        from epmt.epmt_cmd_delete import epmt_delete_jobs
        return not epmt_delete_jobs(args.epmt_cmd_args)

    if args.command == 'list':
        from epmt.epmt_cmd_list import epmt_list
        return not epmt_list(args.epmt_cmd_args)

    if args.command == 'notebook':
        from epmt.epmt_cmd_notebook import epmt_notebook
        return not epmt_notebook(args.epmt_cmd_args)
