...
refmodel_job_associations_table = Table( 'refmodel_job_associations', Base.metadata,
                                         Column( 'jobid', String,
                                                 ...
                                                 primary_key=True ),
                                         Column( 'refmodel_id', Integer,
                                                 ForeignKey('refmodels.id'),
                                                 primary_key=True ) )
...
class Job(with_metaclass(CommonMeta, Base)):
    ...
    hosts = relationship('Host',
                         back_populates='jobs',
                         secondary=host_job_associations_table)
    ref_models = relationship('ReferenceModel',
                              back_populates="jobs",
                              secondary=refmodel_job_associations_table)
...
class Process(with_metaclass(CommonMeta, Base)):
    ...
    numtids = Column(Integer,
                     default=1)
    cpu_time = Column(Float) # exclusive cpu time
    inclusive_cpu_time = Column(Float) # sum of cpu times of all descendants + process_cpu_time
    ...
    gen = Column(Integer)
    exitcode = Column(Integer)
    # for creating process graph, a child process is included in descendants, while parent is included in ancestors
